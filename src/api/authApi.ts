...
const authApi = apiClient.injectEndpoints({
  endpoints: (build) => ({
    getAnonymousToken: build.query<getCustomerTokenResponse, void>({
      query: () => {
        const method = 'POST';
        const url = `/${process.env.REACT_APP_PROJECT_KEY}/anonymous/token?grant_type=client_credentials&scope=${process.env.REACT_APP_SCOPES}`;
    ...
    >({
      query: ({ email, password }) => {
        const method = 'POST';
        const url = `/${process.env.REACT_APP_PROJECT_KEY}/customers/token?grant_type=password&username=${email}&password=${password}&scope=${process.env.REACT_APP_SCOPES}`;
    ...
    getAccessToken: build.query<getAccessTokenResponse, void>({
      query: () => {
        const method = 'POST';
        const url = `/token?grant_type=client_credentials&scope=${process.env.REACT_APP_SCOPES}`;
    ...
    introspectToken: build.query<IntrospectTokenResponse, string>({
      query: (token) => {
        const method = 'POST';
        const url = `/introspect?token=${token}`;
